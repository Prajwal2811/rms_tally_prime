...
class OwnerController extends Controller
{
    ...
    public function syncAll()
    {
        try {
            ...
            // $ledgers = $this->tally->getLedgers();

            // $sales = $this->tally->getSales();

            // $receipts = $this->tally->getReceipts();
    ...
    private function parseTallyXml($xml)
    {
        // Invalid XML entities remove
        $xml = preg_replace('/&#[0-9]+;/', '', $xml);
        $xml = preg_replace('/&#x[0-9A-Fa-f]+;/', '', $xml);

        // Invalid control chars remove
    ...
    public function ledgerInvoices($company, $ledger)
    {
        try {
            ...
            $xml = $this->tally->getLedgerInvoices($company, $ledger);

            $xmlObj = $this->parseTallyXml($xml);

            if (!$xmlObj) {

                return view('owner.tally.invoices', [
                    'company'  => $company,
                    'ledger'   => $ledger,
                    'invoices' => [],
                    'error'    => 'Invalid XML received from Tally'
                ]);
            }

            $invoices = [];

            $vouchers = $xmlObj->xpath('//VOUCHER');
            ...
            if ($vouchers) {
                ...
                foreach ($vouchers as $voucher) {
                    ...
                    $invoices[] = [
                        ...
                        'party'      => (string)($voucher->PARTYLEDGERNAME ?? ''),
                        'amount'     => (string)($voucher->AMOUNT ?? ''),
                    ];
                }
            }

            return view('owner.tally.invoices', compact(
                'company',
                'ledger',
                'invoices'
            ));
    ...
    public function ledgerReceipts($company, $ledger)
    {
        try {
            ...
            $xml = $this->tally->getLedgerReceipts($company, $ledger);

            $xmlObj = $this->parseTallyXml($xml);

            if (!$xmlObj) {

                return view('owner.tally.receipts', [
                    'company'  => $company,
                    'ledger'   => $ledger,
                    'receipts' => [],
                    'error'    => 'Invalid XML received from Tally'
            ...
            return view('owner.tally.receipts', compact(
                'company',
                'ledger',
                'receipts'
            ));
